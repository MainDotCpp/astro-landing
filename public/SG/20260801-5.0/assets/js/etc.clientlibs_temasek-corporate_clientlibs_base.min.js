...
function stickyHeader() {
    ...
    $window.on("scroll", function () {
        ...
        if (windowScrollCurrent <= (headersHeight - 90)) {
...
jQuery(function ($) {
    ...
    var ObjHeader = {
        ...
        searchbox: function () {
            $("header .search-button").on("click", function ( /** !jQuery.Event */ event) {
                event.preventDefault();
        ...
        createMobileMenu: function () {
            ...
            $(".search-row").clone().appendTo($divParent);
            $(".language-switch").clone().appendTo($divParent);
    ...
    function setBgImg() {
        ...
        arrays.forEach(teasers, function(teaser) {
            ...
            if (teaserWrapper && teaserImg) {
                ...
                var isFullBgContent = teaserWrapper.classList.contains("full-bg-content-left") || 
                                      teaserWrapper.classList.contains("full-bg-content-right") || 
                                      teaserWrapper.classList.contains("full-bg-content-full-height");
                var isMobileView = windowWidth < 767;
                var imgSrc = teaserImg.getAttribute("src");

                if (isFullBgContent) {
                    teaserWrapper.style.backgroundImage = "url(" + imgSrc + ")";
                    if (isMobileView && teaserMobileImg) {
                        teaserWrapper.style.backgroundImage = "url(" + teaserMobileImg + ")";
                    }
                } else if (isMobileView && teaserMobileImg) {
...
blacksunplc.events.ready(function () {
    ...
    function initiateAnimation() {
        visible(".homepage .page > .par.parsys > div");
        visible(".full-width-template .page .main-wrapper> .par.parsys > div");
        $(".homepage .image-video-banner").addClass(isVisible);
        $(".visual-story-article-page .image-video-banner").addClass(isVisible);
        $(".content-page.full-width-template .image-video-banner").addClass(isVisible);
        $(".content-page.full-width-template .breadcrumbs.parbase").addClass(isVisible);
    }
    initiateAnimation();
    ...
    $(window).scroll(function(){
        visible(".homepage .page > .par.parsys > div");
        visible(".content-page.full-width-template .page .main-wrapper> .par.parsys > div");
        visible(".visual-story-article-page .page > .par.parsys > .colour-background .colour-par.parsys > div");
        visible(".homepage .image-video-banner");
        visible(".visual-story-article-page .image-video-banner");
        visible(".content-page.full-width-template .image-video-banner");
        visible(".content-page.full-width-template .breadcrumbs.parbase");
        visible(".homepage .articles-carousel");
    });

    /**
     * If the document doesn't have a scrollbar, all elements will be visible.
     */
...
blacksunplc.events.ready(function () {
    ...
    function initSetup() {
        chatRoot = document.getElementById('bs-chat-root');
        const shadow = chatRoot.shadowRoot;
        chatbotBtn = shadow.querySelector('.ow-launcher');
        chatbotDialog = shadow.querySelector('.ow-window');
        isHomepage = document.body.classList.contains('homepage');


        if (!chatbotBtn) { return; }
        parentElement = chatRoot.parentElement;
        const welcomeMessage = shadow.querySelector('.msg-welcome');
        chatbotDialog.appendChild(welcomeMessage);

        // 4. Inject widget.css inside shadow root
        var link = document.createElement('link');
        link.rel = 'stylesheet';
        link.href = '/etc.clientlibs/temasek-corporate/clientlibs/base/resources/chat-widget.css';
        link.onerror = function () {
            console.error('[ChatWidget] Failed to load widget.css — widget may appear unstyled');
        };
        shadow.appendChild(link);
        jsonml.append(chatbotBtn,
            ['',
                ['span', { class: "frame-1" }],
                ['span', { class: "frame-2" }],
                ['span', { class: "frame-3" }],
        ...
        const bannerElement = parentElement.querySelector('.image-video-banner');
        if (bannerElement) {
            parentElement = bannerElement;
        }

        if (document.body.classList.contains('banner-chatbot')) {
            setTimeout(() => {
                BSChat._openWidget();
            }, 100);
        }

        console.log('Chat widget initialized: ', BSChat);
    }
    ...
    function updateButtonPosition() {
        ...
        if (rect.bottom < 0) {
            document.body.classList.remove('banner-chatbot');
            if (chatbotDialog.classList.contains("initial-state")) {
                BSChat._closeWidget();
            }
        } else {
            document.body.classList.add('banner-chatbot');
            if (!chatbotDialog.classList.contains('dirty') && !BSChat._isOpen) {
                BSChat._openWidget();
            }

            if (chatbotDialog.classList.contains('dirty') && !BSChat._isOpen && document.body.classList.contains('banner-chatbot')) {
                BSChat._openWidget();
            }
        }
    }

    window.addEventListener('bschat:WidgetLoaded', (event) => {
    ...
    document.addEventListener('bschat:message-sent', (e) => {
        console.log('Message sent');
        if (!chatbotDialog.classList.contains("dirty")) {
            chatbotDialog.classList.add("dirty");
        }
        chatbotDialog.classList.add("dialog-visible");
        chatbotDialog.classList.remove("initial-state");
    });

    document.addEventListener('bschat:dialog-open', (e) => {
        console.log('Dialog opened');
        if (document.body.classList.contains('banner-chatbot')) {
            chatbotDialog.classList.add("initial-state");
        } else {
            if (!chatbotDialog.classList.contains("dirty")) {
                chatbotDialog.classList.add("initial-state");
            } else {
    ...
    document.addEventListener('bschat:dialog-close', (e) => {
        ...
        chatbotDialog.classList.remove("dialog-visible");
